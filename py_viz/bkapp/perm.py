...
def eval_perm(num_data):
    log, color = 'PERM', 'black'
    df = pd.read_csv(f'py_viz/data/evaluate_{log}.csv').dropna(subset=['WELL'])
    df['abs_error'] = df['error'].abs()
    df = df[['WELL',f'{log}','prediction','DEPTH','abs_error']][df['prediction'].notnull()].reset_index(drop=True)
    ...
    # track: PERM (m3/m3)
    a, b = 0.1, 10000
    p1 = figure(width=int(wdt*1.15), height=hgh, x_range=[a, b], y_range=[d2, d1], x_axis_type='log',
    ...
    p1.add_layout(LogAxis(x_range_name=f'{log}', axis_label=f'{log}'), 'above')
    ...
    <div>
        ...
        <span style="font-size: 8px; color: #0CFF0C;"><b>PERM:</b> @PERM{0.2f}</span><br>
    ...
    p2.add_layout(LogAxis(x_range_name='error', axis_label='abs_error'), 'above')
    p2.hbar(y='index', left=0.0000001, right='abs_error',height=0.02, source=source, color='color')
    ...
    p3 = figure(width=400, height=400, x_range=[a,b], y_range=[a,b], title='Predicted - Actual Data Distribution',
                tools=tools, y_axis_label='Prediction' ,x_axis_label='Actual', x_axis_type='log', y_axis_type='log')
    ...
    p3.xgrid.minor_grid_line_alpha = 0.05
    ...
    <div>
        ...
        <span style="font-size: 8px;"><b>DEPTH:</b> @DEPTH{0.2f}</span><br>
        <span style="font-size: 8px;"><b>index:</b> @index</span><br>
        <span style="font-size: 8px;"><b>PERM:</b> @PERM{0.2f}</span><br>
